...
describe('FoodSearch', () => {
  ...
  describe('when user fill input', () => {
    ...
    describe('and API return result', () => {
      ...
      it('should add `foods` in state', () => {
        expect(wrapper.state().foods).toEqual(result)
      })
      it('should display `foods`', () => {
        console.log(wrapper.find('table tbody tr'))
        expect(wrapper.find('table tbody tr').length).toBe(2)
      })
      it('should display descriptions', () => {
        expect(wrapper.find('table tbody tr').first().find('td').first().text()).toBe('food1')
      })
    })
    
    describe('and then user clear input field', () => {
      beforeEach(() => {
        client.getFoods.mockReturnValue({then: cb => cb([])})
